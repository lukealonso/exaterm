...
use exaterm_ui::layout::{card_layout, card_terminal_slot_rect, CardRect, MARGIN};
            ...
            if let Some(session_id) = session_at_point(self.frame(), point) {
                INTERACTION.with(|slot| {
                    if let Some(handler) = slot.borrow().as_ref() {
                        handler(BattlefieldInteraction::Select(session_id));
                    }
                });
...
fn draw_status_chip(
    label: &str,
    status: exaterm_ui::supervision::SessionTileStatus,
    x: f64,
    y_cursor: &mut f64,
    render: &TerminalRenderState,
) {
